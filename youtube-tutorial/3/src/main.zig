...
fn foobarCb(demo_widget: *DemoWidget, user_data: c.gpointer) void {
    _ = user_data;
    _ = demo_widget;
...
fn activateCb(app: *c.GtkApplication, user_data: ?*anyopaque) void {
    ...
    if (c.gtk_application_window_new(app)) |window_widget| {
        ...
        if (DemoWidget.new()) |demo_widget| {
            ...
            _ = c.g_signal_connect_data(demo_widget, "foobar", @ptrCast(c.GCallback, &foobarCb), null, null, c.G_CONNECT_DEFAULT);
