...
pub const DemoWidget = opaque {
    ...
    extern fn demo_widget_get_timeout(self: *Self) c.guint;
    const getTimeout = demo_widget_get_timeout;

    extern fn demo_widget_set_timeout(self: *Self, timeout: c.guint) void;
    const setTimeout = demo_widget_set_timeout;
    ...
    fn foobarSignalSourceFunc(optional_user_data: c.gpointer) callconv(.C) c.gboolean {
        if (optional_user_data) |user_data| {
            const self = alignPtrCast(*Self, user_data);
            c.g_signal_emit(self, signals[@enumToInt(Signals.foobar)], 0);
        }
        return @boolToInt(c.G_SOURCE_CONTINUE);
    }
    ...
    export fn demo_widget_init(self: *Self) void {
        ...
        if (optional_button) |button| {
            ...
            self.setTimeout(c.g_timeout_add(5000, &foobarSignalSourceFunc, self));
        }
    }
    ...
    fn finalize(optional_object: ?*c.GObject) callconv(.C) void {
        if (optional_object) |object| {
            const self = alignPtrCast(*Self, object);
            _ = c.g_source_remove(self.getTimeout());
    ...
    export fn demo_widget_class_init(class: *c.DemoWidgetClass) void {
        ...
        signals[@enumToInt(Signals.foobar)] = c.g_signal_new_class_handler(
            "foobar",
